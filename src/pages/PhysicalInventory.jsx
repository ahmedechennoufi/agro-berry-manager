...
const PhysicalInventory = () => {
  ...
  const [filterDiff, setFilterDiff] = useState('ALL'); // ALL, ecart, ok, manquant, excedent
  ...
  const comparisonData = useMemo(() => {
    return theoreticalStock
      .map(item => {
        ...
        let status = 'pending'; // not yet entered
        if (hasPhysical) {
  ...
  // Reset physical stock
  const handleReset = () => {
    if (window.confirm('Effacer toutes les saisies de stock physique ?')) {
      setPhysicalStock({});
    }
  };

  // Copy theoretical to physical (pre-fill)
  const handlePrefill = () => {
    if (window.confirm('Pré-remplir avec le stock théorique ? Vous pourrez modifier les valeurs ensuite.')) {
      ...
      theoreticalStock.forEach(item => {
        prefilled[item.name] = item.theoretical.toString();
      });
  ...
  const handleExport = () => {
    ...
    const lines = [
      ...
      `Avec écart: ${stats.withDiff}`,
      `Valeur écart total: ${fmtMoney(stats.totalDiffValue)}`
    ];
  ...
  const getStatusBadge = (status, diff, diffPercent) => {
    switch (status) {
      case 'ok':
        return <span className="px-2 py-1 rounded-lg bg-green-100 text-green-700 text-xs font-bold">✅ OK</span>;
      case 'manquant':
        return <span className="px-2 py-1 rounded-lg bg-red-100 text-red-700 text-xs font-bold">🔻 {fmt(Math.abs(diff))} ({fmt(Math.abs(diffPercent))}%)</span>;
      case 'excedent':
        return <span className="px-2 py-1 rounded-lg bg-blue-100 text-blue-700 text-xs font-bold">🔺 +{fmt(diff)} (+{fmt(diffPercent)}%)</span>;
      default:
        return <span className="px-2 py-1 rounded-lg bg-gray-100 text-gray-500 text-xs">⏳ En attente</span>;
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        {selectedFarm && stats.entered > 0 && (
          <div className="flex gap-2">
            ...
            <button
              onClick={handleReset}
              className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-600 rounded-xl font-medium transition-colors text-sm"
            >
              🗑️ Réinitialiser
            </button>
          </div>
      ...
      {!selectedFarm ? (
        <Card className="p-12">
          <EmptyState icon="🏭" message="Sélectionnez une ferme pour commencer l'inventaire" />
        </Card>
      ) : (
        <>
          {/* Progress & Stats */}
                    ...
                    {comparisonData.map((item, idx) => (
                      <tr 
                        ...
                        className={`border-b transition-colors ${
                          ...
                          item.status === 'ok' ? 'bg-green-50/50 hover:bg-green-50' :
                          'hover:bg-gray-50'
                        }`}
                      >
                        ...
                        <td className="p-3 text-center">
                          <input
                            ...
                            step="0.01"
                            min="0"
                            value={physicalStock[item.name] ?? ''}
                            onChange={(e) => handlePhysicalChange(item.name, e.target.value)}
                            placeholder="—"
                        ...
                        <td className={`p-4 text-center font-bold ${
                          item.diff === null ? 'text-gray-400' :
                          item.diff < -0.01 ? 'text-red-600' :
                          item.diff > 0.01 ? 'text-blue-600' :
                          'text-green-600'
                        }`}>
                          {item.diff === null ? '—' : 
                        ...
                        <td className={`p-4 text-right font-medium ${
                          item.diffValue === null ? 'text-gray-400' :
                          item.diffValue < 0 ? 'text-red-600' :
                          item.diffValue > 0 ? 'text-blue-600' :
                          'text-green-600'
                        }`}>
                          {item.diffValue === null ? '—' : fmtMoney(item.diffValue)}
                        </td>
                        <td className="p-4 text-center">
                          {getStatusBadge(item.status, item.diff, item.diffPercent)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
